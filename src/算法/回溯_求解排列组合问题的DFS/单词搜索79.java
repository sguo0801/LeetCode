...
public class 单词搜索79 {
    class Solution {
        private int[][] next = {{1, 0}, {-1, 0}, {0, 1}, {0, -1}};  //方向迭代.
        int m, n;  //作为dps的边界
        public boolean exist(char[][] board, String word) {
            ...
            for(int i = 0; i < m; i++){
                for(int j = 0; j < n; j++){
                    if(dps(board, word, i, j, 0)){  //从这里开始的都是放进去第一个字母,但是还没有确认是不是符合要求的头.
                        return true;
                    }
                }
            }
        ...
        private boolean dps(char[][] board, String word, int i, int j, int num){
            ...
            if(i < 0 || i == m || j < 0 || j == n || (board[i][j] != word.charAt(num))){  //注意别马虎,board[i][j]
                return false;
            }
