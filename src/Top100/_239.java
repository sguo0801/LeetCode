package Top100;
//读者可能疑惑，push 操作中含有 while 循环，时间复杂度不是 O(1)O(1) 呀，那么本算法的时间复杂度应该不是线性时间吧？
//单独看 push 操作的复杂度确实不是 O(1)O(1)，但是算法整体的复杂度依然是 O(N)O(N) 线性时间。
// 要这样想，nums 中的每个元素最多被 push_back 和 pop_back 一次，没有任何多余操作，所以整体的复杂度还是 O(N)O(N)。
